...
public class AppuserController extends BaseController {
	...
	@RequestMapping(value = "/userDetailById")
	public ModelAndView userDetailById() throws Exception {
		if (!Jurisdiction.buttonJurisdiction(menuUrl, "add")) {
			return null;
		} // 校验权限
		ModelAndView mv = this.getModelAndView();
		...
		pd.put("ROLE_ID", "2");
		List<Role> roleList = roleService.listAllRolesByPId(pd); // 列出会员组角色
		PageData user = appuserService.findByUiId(pd);
	...
	public ModelAndView editS(HttpServletRequest req) throws Exception {
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		String p = req.getParameter("USER_ID");
		...
		appuserService.editS(pd);
		pd.remove("STATUS");
		pd.remove("USER_ID");
		mv.addObject("msg", "success");
		mv.setViewName("forward:/happuser/listUsers.do");
		return mv;
	}
