...
/**
 * 总入口
 * @author fh QQ 3 1 3 5 9 6 7 9 0[庸人修行录]
 * 修改日期：2015/11/2
 */
/**
...
public class LoginController extends BaseController {
	...
	public Object login()throws Exception{
		...
		pd = this.getPageData();
		String errInfo = "";
		String KEYDATA[] = pd.getString("KEYDATA").replaceAll("qq728971035fh", "").replaceAll("QQ728971035fh", "").split(",fh,");
		...
		if(null != KEYDATA && KEYDATA.length == 3){
			...
			if(null == code || "".equals(code)){//判断效验码
				...
				if(Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)){		//判断登录验证码
					...
					tdUser.setMemberid(USERNAME);
			
					TdUser beforeTdUser =clientUserService.queryOne(tdUser);
			 if(beforeTdUser!=null) {
				
						TdOrganizerInfo tdOrganizerInfo = new TdOrganizerInfo();
						tdOrganizerInfo.setUserId(beforeTdUser.getUserId());
						tdOrganizerInfo.setPassword(passwd);
						TdOrganizerInfo loginTdOrganizerInfo  = organizerInfoService.queryOne(tdOrganizerInfo);
			 if(loginTdOrganizerInfo!=null) {
					
						this.removeSession(USERNAME);//清缓存
						pd.put("LAST_LOGIN",DateUtil.getTime().toString());
/*						userService.updateLastLogin(pd);//更新登录时间
*/						User user = new User();
						user.setUSER_ID(loginTdOrganizerInfo.getOrganizerInfoId());
						user.setUSERNAME(loginTdOrganizerInfo.getUserId());
				
						user.setPASSWORD(loginTdOrganizerInfo.getPassword());
						user.setNAME(loginTdOrganizerInfo.getName());
						user.setRIGHTS("22592111669665739975592870737637022906810831294812620197467215446901550642889587999246991367961839975767182923986271972624986374927027127581012424707895568851446368731861728964581056585258540204929722367445904484047257600");
						user.setROLE_ID("779de1646080471b9133b357b23cf93f");
						SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
						String loginTime = sdf.format(loginTdOrganizerInfo.getUpdated());
						user.setLAST_LOGIN(loginTime);
						user.setSTATUS("0");
						session.setAttribute(Const.SESSION_USER, user);			//把用户信息放session中
						session.removeAttribute(Const.SESSION_SECURITY_CODE);	//清除登录验证码的session
						//shiro加入身份验证
		...
		try{
			...
			if (user != null) {
				...
				if(null == userr){
					...
					TdOrganizerInfo loginTdOrganizerInfo  = organizerInfoService.queryById(user.getUSER_ID());
					user.setUSER_ID(loginTdOrganizerInfo.getOrganizerInfoId());
					user.setUSERNAME(loginTdOrganizerInfo.getUserId());
					user.setPASSWORD(loginTdOrganizerInfo.getPassword());
					user.setNAME(loginTdOrganizerInfo.getName());
					...
					user.setLAST_LOGIN(loginTime);
					user.setSTATUS("0");
					Role role =roleService.getRoleById("779de1646080471b9133b357b23cf93f");
					user.setRole(role);
					/*	
	...
	public Map<String, String> getUQX(String USERNAME){
		...
		try {
			...
			pd.put("ROLE_ID", "779de1646080471b9133b357b23cf93f");//获取角色ID
