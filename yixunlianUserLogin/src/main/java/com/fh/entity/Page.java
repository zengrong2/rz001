...
public class Page {
	...
	private String pageStrPartenr;
	
	public void setPageStrPartenr(String pageStrPartenr) {
		this.pageStrPartenr = pageStrPartenr;
	}
	...
	//拼接分页 页面及JS函数
		public String getPageStrPartenr() {
			StringBuffer sb = new StringBuffer();
			if(totalResult>0){
				sb.append("	<ul class=\"pagination pull-right no-margin\">\n");
				if(currentPage==1){
				...
				sb.append(" top.jzts();");
				sb.append("	if(true && document.forms[0]){\n");
				sb.append("		var url = \"user/listPartenr.do?type=1\";\n");
				sb.append("		if(url.indexOf('?')>-1){url += \"&"+(entityOrField?"currentPage":"page.currentPage")+"=\";}\n");
				sb.append("		else{url += \"?"+(entityOrField?"currentPage":"page.currentPage")+"=\";}\n");
				sb.append("		url = url + page + \"&" +(entityOrField?"showCount":"page.showCount")+"="+showCount+"\";\n");
				sb.append("		document.forms[0].action = url;\n");
