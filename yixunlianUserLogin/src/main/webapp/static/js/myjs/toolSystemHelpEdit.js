...
/*	if($("#EMAIL").val()==""){
	...
	if($("#system_help_type").val()==""){
		$("#system_help_type").tips({
			side:3,
            msg:'请输入系统帮助类型',
            bg:'#AE81FF',
            time:2
        });
		$("#system_help_type").focus();
		return false;
	}
	if($("#CONTENT").val()==""){
	...
	var system_help_type = $("#system_help_type").val();
	var system_help_id = $("#system_help_id").val();
	var TYPE  = $("#TYPE").val();
	/*var TITLE = $("#TITLE").val();*/
	...
	$.ajax({
		type: "POST",
		url: locat+'/systemHelp/sendSystemHelpEdit.do?tm='+new Date().getTime(),
    	data: {TYPE:TYPE,CONTENT:CONTENT,fmsg:fmsg,system_help_type:system_help_type,system_help_id:system_help_id},
		dataType:'json',
