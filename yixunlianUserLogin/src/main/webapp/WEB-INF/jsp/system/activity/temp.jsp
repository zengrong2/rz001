							...
							<c:choose>
								<c:when test="${not empty listOrganizerInfo}">
									<c:forEach items="${listOrganizerInfo}" var="ts" varStatus="vs">
										...
										<td class="center" style="cursor:pointer;color:blue;" onClick="toActivityEnrollDetail('${ts.activity_id}')">详情</td>
										<td class='center'>￥${ts.sumSoloActivityDealMoney }</td>
										<td class="center">
										<c:if test="${ts.activity_status==0 ||ts.activity_status==null}">
										未开始
										</c:if>
										<c:if test="${ts.activity_status== 1 }">
										活动进行中
										</c:if>
										<c:if test="${ts.activity_status== 2 }">
										活动已经结束
										</c:if>
