			...
			<div class="main-content-inner">
							...
							<thead>
								<tr>
									...
									<!--    <th class='center'>主办方</th> -->
										<!-- <th class='center' style="width:80px;">活动详情</th> -->
										<!-- <th class='center'>开始时间</th>
									    <th class='center'>结束时间</th> -->
									    <th class='center' style="width:180px;">活动时间</th>
									    <th class='center'>客户匹配</th>
									   <!--  <th class='center'>提成项目</th> -->
									   <!--  <th class='center'>报名开始</th>
										<th class='center'>报名结束</th> -->
										<th class='center'>报名列表</th> 
										<th class='center'>签到人数</th> 
										<th class='center'>成交人数</th> 
										<th class='center'>成交金额</th> 
										<th class='center'>应发提成金额</th> 
										<th class='center'>待发提成金额</th>
							...
							<c:choose>
								<c:when test="${not empty listOrganizerInfo}">
									<c:forEach items="${listOrganizerInfo}" var="ts" varStatus="vs">
										...
										<td class='center'>
										${ts.activitystart_time }<br/>
										${ts.activityend_time }</td>
										<!-- 客户匹配 -->
										<td class="center">
										${ts.matchPersonCount }
										<%-- <div style="cursor:pointer;color:blue;margin-left:10px;" onClick="toActivityDetail('${ts.activity_id}')">
									    ${ts.matchPersonCount }</div> --%>
									    </td>
										
										<td class="center" style="cursor:pointer;color:blue;" onClick="toActivityEnrollDetail('${ts.activity_id}')">${ts.countEnrollPerson }</td>
										<td class="center">${ts.countEnrollSignInPerson }</td>
										<td class="center">${ts.countEnrollDealPerson }</td>
										<td class='center'>￥${ts.sumSoloActivityDealMoney }</td>
										<td class='center'>￥${ts.shouldPubMoney }</td>
										<td class='center'>￥${ts.sumSoloActivityWaitPayExtract }</td>
			   ...
			   <span style="cursor:pointer;margin-left:10px;" >已上架</span>
			    ...
			    <span style="cursor:pointer;color:blue;margin-left:10px;" onClick="SoloActivityUnderReasonList('${ts.activity_id}')">管理员下架</span>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.del == 1 }">
												 	...
												 	<a class="btn btn-xs btn-success" title="编辑" onclick="editT('${ts.activity_id}');">
									    ...
									    <c:if test="${ts.online_status==4}">
										&nbsp;&nbsp;<a class="label label-success arrowed" <%-- onclick="changeState('${ts.activity_id}','${ts.online_status}');" --%>>草稿</a>
									    </c:if>
									     <c:if test="${ts.online_status==3}">
										&nbsp;&nbsp;<a class="label label-info arrowed" >已被管理员下架</a>
									    </c:if>
													</c:if>
												</div>		
...
function add(){
	 ...
	 diag.Drag=true;
	 diag.Title ="新增";
	 diag.URL = '<%=basePath%>organizerInfo/goOrganizerInfoAddActivity.do';
	 diag.Width = 1000;
	 diag.Height = 600;
	 diag.CancelEvent = function(){ //关闭事件
		 window.location.reload();
...
function editT(activity_id){
	 ...
	 diag.Drag=true;
	 diag.Title ="修改";
	 diag.URL = '<%=basePath%>organizerInfo/goOrgEditActivity.do?activity_id='+activity_id;
	 diag.Width = 1000;
	 diag.Height = 600;
	 diag.CancelEvent = function(){ //关闭事件
		 window.location.reload();
		...
		if(result) {
			...
			if(str==''){
				...
				if(msg == '确定要删除选中的数据吗?'){
					...
					$.ajax({
						type: "POST",
						url: '<%=basePath%>organizerInfo/deleteAllActivity.do?tm='+new Date().getTime(),
