						...
						<form action="happuser/${msg }.do" name="userForm" id="userForm" method="post">
							...
							<table id="table_report" class="table table-striped table-bordered table-hover">
								<tr>
									...
									<td><input type="tel" name="PHONE" id="PHONE" value="${pd.PHONE }" placeholder="这里输入手机号" title="手机号" style="width:98%;" readOnly="true"/></td>
								</tr>
								<tr>
									<td style="width:79px;text-align: right;padding-top: 13px;">会员类别:</td>
									<td id="js">
								...
								<tr>
									<td class="center" colspan="6">
										<a class="btn btn-mini btn-primary" onclick="savse();">保存</a>
...
<script type="text/javascript">
	...
	function ismail(mail){
		...
		function save() {
			...
			var ROLE_ID=$("#ROLE_ID").val();//会员类别
			var moeny=$("#moeny").val();//充值金额
			var USERNAME=$("#USERNAME").val();//用户名
			var orderno=$("#NUMBER").val();//编号
			var password=$("#password").val();//密码
			var SFID=$("#SFID").val();//身份证号
			var STATUS=$("#STATUS").val();//状态
			var BZ=$("#BZ").val();//备注
			var created = new Date();
			$.ajax({
				url:'<%=path%>/app/user/postuser',
				type : 'POST',
				data : {
				    name : name,
				    phone : phone,
