				 	...
				 	<div class="span12">
						<div class="widget-box">
							<div class="widget-header widget-header-blue widget-header-flat wi1dget-header-large">
								<c:if test="${tdSystembulletin.systemStatus ==0 }">
								<h4 class="lighter">编辑活动功能介绍和了解我们</h4>
								</c:if>
								<c:if test="${tdSystembulletin.systemStatus ==1 }">
								<h4 class="lighter">收益攻略</h4>
								</c:if>
								<c:if test="${tdSystembulletin.systemStatus ==2 }">
								<h4 class="lighter">用户协议</h4>
								</c:if>
								<c:if test="${tdSystembulletin.systemStatus ==3 }">
								<h4 class="lighter">资源攻略</h4>
								</c:if>
								<c:if test="${tdSystembulletin.systemStatus ==4 }">
								<h4 class="lighter">关于我们</h4>
								</c:if>
								<c:if test="${tdSystembulletin.systemStatus ==5 }">
								<h4 class="lighter">体验vip的培训</h4>
								</c:if>
							</div>
							<div class="widget-body">
							 ...
							 <div class="step-content row-fluid position-relative">
								...
								<input type="hidden" name="systembulletinId" id="systembulletinId" value="${tdSystembulletin.systembulletinId}"/>
								<input type="hidden" name="systemStatus" id="systemStatus" value="${tdSystembulletin.systemStatus}"/>
							     <span style="font-weight:bold;">分享标题：</span>	<input type="text" name="shareTitle" id="shareTitle" value="${tdSystembulletin.shareTitle }" placeholder="请输入分享标题，15字之内" maxlength="15"style="width:220px;"/>
							     <span style="font-weight:bold;">分享摘要：</span>	<input type="text" name="shareSummary" id="shareSummary" value="${tdSystembulletin.shareSummary }" maxlength="40" placeholder="请输入分享摘要，40字之内" style="width:220px;"/>
								<table style="width:100%;" id="xtable">
									...
									<tr>
										<td style="text-align: center;padding-top: 5px;">
											...
											<label style="float:left;padding-left: 32px;"><input name="form-field-radio" id="form-field-radio1" onclick="setType('1');"  type="radio" value="icon-edit" class="ace"><span class="lbl">纯文本</span></label>
...
<!--引入属于此页面的js -->
<script type="text/javascript" src="static/js/myjs/toolSystembulletin.js"></script>	
