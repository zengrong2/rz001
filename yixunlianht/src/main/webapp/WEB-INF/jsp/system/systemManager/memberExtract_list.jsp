					...
					<div class="row">
					   ...
					   --%>
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center" style="width:60px;">编号</th>
									    <th class='center'>帮助类型</th>
									
									    <th class='center'>帮助内容</th>
							...
							<!-- 开始循环 -->	
							<c:choose>
								<c:when test="${not empty listSystemHelp}">
									<c:forEach items="${listSystemHelp}" var="ts" varStatus="vs">
										<tr>
											...
											<td class='center'>${ts.system_help_type}</td>
											<td class="center" >
										   <div style="cursor:pointer;color:blue;margin-left:10px;" onClick="toSystemHelpDetail('${ts.system_help_id}')">预览</div>										
										
										    </td>
										  <%--   <td class='center'>${ts.system_help_context}</td> --%>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-xs btn-success" title="编辑" onclick="editT('${ts.system_help_id}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													<a class="btn btn-xs btn-danger" onclick="delSystemHelp('${ts.system_help_id}');">
...
function toSystemHelpDetail(system_help_id){
   top.jzts();
	 var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="系统帮助详情";
	 diag.URL = '<%=basePath%>systemHelp/showSystemHelpDetailById?system_help_id='+system_help_id;
	 diag.Width =800;
	 diag.Height=600;
...
//删除
function delSystemHelp(system_help_id){
	bootbox.confirm("确定要删除吗?", function(result) {
		if(result) {
			top.jzts();
			var url = "<%=basePath%>systemHelp/delSystemHelpById.do?system_help_id="+system_help_id;
			$.get(url,function(data){
				window.location.reload();
			});
		};
...
function add(){
	 ...
	 diag.Drag=true;
	 diag.Title ="新增";
	 diag.URL = '<%=basePath%>systemHelp/goAddSystemHelp.do';
	 diag.Width = 1000;
	 diag.Height = 600;
	 diag.CancelEvent = function(){ //关闭事件
		 window.location.reload();
...
function editT(system_help_id){
	 top.jzts();
	 var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="修改";
	 diag.URL = '<%=basePath%>systemHelp/goEditSystemHelp.do?system_help_id='+system_help_id;
	 diag.Width = 1000;
	 diag.Height = 600;
	 diag.CancelEvent = function(){ //关闭事件
		 window.location.reload();
