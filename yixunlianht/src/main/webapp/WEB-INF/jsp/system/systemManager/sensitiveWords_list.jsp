			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
					   ...
					   --%>
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
							...
							<!-- 开始循环 -->	
							<c:choose>
								<c:when test="${not empty SensitiveWords}">
									<c:forEach items="${SensitiveWords}" var="ts" varStatus="vs">
										<tr>
										...
										<%-- 	<td class='center' style="width: 30px;">
											...
											<td class='center' style="height:400px;" colspan="2">${ts.sensitive_words_context}
											
											<c:if test="${QX.del == 1 }">
											<div style="margin-left:150px;margin-top:-65px;">		
													<a class="btn btn-xs btn-success" title="编辑" onclick="editT('${ts.sensitive_words_id}');">
					...
					<table style="width:100%;">
						...
						<c:if test="${ empty SensitiveWords }">
							<c:if test="${QX.add == 1 }">
...
function add(){
	 ...
	 diag.Drag=true;
	 diag.Title ="新增";
	 diag.URL = '<%=basePath%>sensitiveWords/goAddSensitiveWords.do';
	 diag.Width = 1000;
	 diag.Height = 600;
	 diag.CancelEvent = function(){ //关闭事件
		 window.location.reload();
...
function editT(sensitive_words_id){
	 top.jzts();
	 var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="修改";
	 diag.URL = '<%=basePath%>sensitiveWords/goEditSensitiveWords.do?sensitive_words_id='+sensitive_words_id;
	 diag.Width = 1000;
	 diag.Height = 600;
	 diag.CancelEvent = function(){ //关闭事件
		 window.location.reload();
