					...
					<div class="row">
					   ...
					   --%>
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center" style="width:60px;">编号</th> 
									    <th class='center'>系统信息类型</th>
									
									    <th class='center'>系统信息内容</th>
							...
							<!-- 开始循环 -->	
							<c:choose>
								<c:when test="${not empty listSystembulletin}">
									<c:forEach items="${listSystembulletin}" var="ts" varStatus="vs">
										<tr>
											...
											<c:if test="${ts.system_status ==0 }">
											<td class='center'>活动功能介绍和了解我们</td>
											</c:if>
											
											<c:if test="${ts.system_status ==1 }">
											<td class='center'>收益攻略</td>
											</c:if>
											<c:if test="${ts.system_status ==2 }">
											<td class='center'>用户协议</td>
											</c:if>
											<c:if test="${ts.system_status ==3 }">
											<td class='center'>资源攻略</td>
											</c:if>
											<c:if test="${ts.system_status ==4 }">
											<td class='center'>关于我们</td>
											</c:if>
											<c:if test="${ts.system_status ==5 }">
											<td class='center'>体验vip的应用培训</td>
											</c:if>
										  <%--   <td class='center'>${ts.systembulletin_url}</td> --%>
										<td class="center" >
										<div style="cursor:pointer;color:blue;margin-left:10px;" onClick="toSystembulletinDetail('${ts.systembulletin_id}')">预览</div>										
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-xs btn-success" title="编辑" onclick="editT('${ts.systembulletin_id}');">
...
function toSystembulletinDetail(systembulletin_id){
   top.jzts();
	 var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="活动详情";
	 diag.URL = '<%=basePath%>systembulletin/showSystembulletinDetailById?systembulletin_id='+systembulletin_id;
	 diag.Width =800;
	 diag.Height=600;
...
function editT(systembulletin_id){
	 top.jzts();
	 var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="修改";
	 diag.URL = '<%=basePath%>systembulletin/goEditSystembulletin.do?systembulletin_id='+systembulletin_id;
	 diag.Width = 1000;
	 diag.Height = 600;
	 diag.CancelEvent = function(){ //关闭事件
		 window.location.reload();
