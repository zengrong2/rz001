					...
					<div class="row">
					   ...
					   --%>
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center" style="width:60px;">编号</th>
									    <th class='center'>系统个税+手续费(百分比)</th>
							...
							<!-- 开始循环 -->	
							<c:choose>
								<c:when test="${not empty listTaxrevenue}">
									<c:forEach items="${listTaxrevenue}" var="ts" varStatus="vs">
										<tr>
											...
											<td class='center'>${ts.taxrevenue_rate}</td>
											<c:if test="${ ts.taxrevenue_status==1}">
											<td class='center'>当前税率</td>
									     	</c:if>
									     	<c:if test="${ ts.taxrevenue_status!=1}">
											<td class='center'>往期税率</td>
									     	</c:if>
										<td class='center'>${ts.created}</td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.del == 1 }">
													<c:if test="${ ts.taxrevenue_status==1}">
													<a class="btn btn-xs btn-success" title="编辑" onclick="editT('${ts.taxrevenue_id}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													</c:if>
													<%-- <c:if test="${ ts.taxrevenue_status!=1}">
													<a class="btn btn-xs btn-success" title="往期不可修改编辑" onclick="editT('${ts.taxrevenue_id}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="往期不可修改编辑"></i>
													</a>
													</c:if> --%>
													
													<c:if test="${ ts.taxrevenue_status!=1}">
													<a class="btn btn-xs btn-danger" onclick="delTaxRevenue('${ts.taxrevenue_id}');">
...
//删除
function delTaxRevenue(taxrevenue_id){
	bootbox.confirm("确定要删除吗?", function(result) {
		if(result) {
			top.jzts();
			var url = "<%=basePath%>taxrevenue/delTaxRevenueById.do?taxrevenue_id="+taxrevenue_id;
			$.get(url,function(data){
				window.location.reload();
			});
		};
...
function add(){
	 ...
	 diag.Drag=true;
	 diag.Title ="新增";
	 diag.URL = '<%=basePath%>taxrevenue/goAddTaxrevenue.do';
	 diag.Width = 600;
	 diag.Height = 300;
	 diag.CancelEvent = function(){ //关闭事件
		 window.location.reload();
...
function editT(taxrevenue_id){
	 top.jzts();
	 var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="修改";
	 diag.URL = '<%=basePath%>taxrevenue/goEditTaxRevenue.do?taxrevenue_id='+taxrevenue_id;
	 diag.Width = 500;
	 diag.Height = 510;
	 diag.CancelEvent = function(){ //关闭事件
		 window.location.reload();
