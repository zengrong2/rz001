					...
					<div class="row">
							...
							<tr>
								<td>
									...
									<span class="input-icon">
										<input class="nav-search-input" autocomplete="off" id="nav-search-input" type="text" name="keywords" value="${pd.keywords }" placeholder="这里输入后台管理员名字" />
					   ...
					   --%>
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center" style="width:60px;">编号</th>
									    <th class='center'>后台操作人员</th>
									
									    <th class='center'>操作事件</th>
							...
							<!-- 开始循环 -->	
							<c:choose>
								<c:when test="${not empty listSysLog}">
									<c:forEach items="${listSysLog}" var="ts" varStatus="vs">
										<tr>
										
											<td class='center' style="width: 30px;">
												<label><input type='checkbox' name='ids' value="${ts.FHLOG_ID }"   class="ace"/><span class="lbl"></span></label>
											</td>
											<td class="center">${vs.index+1}</td>
											<td class='center'>${ts.USERNAME}</td>
										    <td class='center'>${ts.CONTENT}</td>
										    <td class='center'>${ts.CZTIME}</td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-xs btn-danger" onclick="delSysLOG('${ts.FHLOG_ID}');">
...
//删除
function delSysLOG(FHLOG_ID){
	bootbox.confirm("确定要删除吗?", function(result) {
		if(result) {
			top.jzts();
			var url = "<%=basePath%>systemLog/delSystemLogById.do?FHLOG_ID="+FHLOG_ID;
			$.get(url,function(data){
				window.location.reload();
			});
		};
		...
		if(result) {
			...
			if(str==''){
				...
				if(msg == '确定要删除选中的数据吗?'){
					...
					$.ajax({
						type: "POST",
						url: '<%=basePath%>systemLog/deleteAllSystemLog.do?tm='+new Date().getTime(),
				    	data: {SystemLog_IDS:str},
						dataType:'json',
