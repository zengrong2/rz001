...
<body class="no-skin">
						...
						<table style="margin-top:5px;">
							<tr>
								...
								<!--  <td style="vertical-align:top;padding-left:2px;">
								 	<select class="chosen-select form-control" name="ROLE_ID" id="role_id" data-placeholder="会员类别" style="vertical-align:top;width: 120px;">
									<option value=""></option>
								...
								<c:if test="${QX.cha == 1 }">
								<td style="vertical-align:top;padding-left:2px;"><a class="btn btn-light btn-xs" onclick="searchs();"  title="检索"><i id="nav-search-icon" class="ace-icon fa fa-search bigger-110 nav-search-icon blue"></i></a></td></c:if>
							</tr>
						</table>
						<!-- 检索  -->
								...
								<c:when test="${not empty userList}">
									...
									<c:forEach items="${userList}" var="user" varStatus="vs">
										<tr>
											...
											<td class="center" style="cursor:pointer;color:blue;" onClick="toUserDetail('${user.USER_ID}')">${user.USERNAME }—查看</td>
											<td class="center">${user.NAME }</td>
											<td class="center">${user.sex==1?"男":"女"}</td>
	...
	<script type="text/javascript" src="static/js/jquery.tips.js"></script>
 	...
 	<script type="text/javascript">
           ...
           $("input[name=fileString]").on('change', function(e) {  
              var name = e.currentTarget.files[0].name;
              

                $.ajax({
                    type:'POST',
                    url:'<%=basePath%>/app/user/excelLoadFile',
                    data:{
                       filePath:name
                         },
                     success:function(){
    ...
    function openLoad(){
			 ...
			 diag.Drag=true;
			 diag.Title ="用户账户";
			 diag.URL = '<%=basePath%>page/system/appuser/user_excel_load';
			 diag.Width = 860;
			 diag.Height = 470;
		...
		function delUser(userId,msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>happuser/deleteU.do?USER_ID="+userId;
