			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="school/listSchool.do" method="post" name="userForm" id="userForm">
							...
							<tr>
								<td>
									...
									<span class="input-icon">
										<input class="nav-search-input" autocomplete="off" id="nav-search-input" type="text" name="keywords" value="${pd.keywords }" placeholder="这里输入学校名称" />
					...
					<td style="vertical-align:top;padding-left:2px;">
                        ...
                        <td>
									<div class="nav-search">
									<span class="input-icon">
										<select class="chosen-select form-control"   id="nav-search-input" name="w_id" id="w_id" data-placeholder="岗前培训学校" style="vertical-align:top;width: 150px;">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center">学校类型</th>
									<th class="center">校长</th>				
									<th class="center">类型</th>
									<th class="center">专业</th>
									<th class="center">学校等级</th>
							...
							<c:choose>
								<c:when test="${not empty sList}">
									<c:forEach items="${sList}" var="school" varStatus="vs">
										<tr>
										<!-- 
										<td class='center' style="width: 30px;">
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.del == 1 }">
													<a class="btn btn-xs btn-danger" onclick="delSchool('${school.school_id}');">
...
function delSchool(schoolId){
	bootbox.confirm("确定要删除吗?", function(result) {
		if(result) {
			top.jzts();
			var url = "<%=basePath%>school/deleteS.do?school_id="+schoolId;
			$.get(url,function(data){
				window.location.reload();
			});
		};
...
function add(){
	 ...
	 diag.Drag=true;
	 diag.Title ="新增";
	 diag.URL = '<%=basePath%>school/goAddS.do';
	 diag.Width = 850;
	 diag.Height = 650;
	 diag.CancelEvent = function(){ //关闭事件
		 window.location.reload();
