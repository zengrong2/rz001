			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="train/goListTrainingAgency.do" method="post" name="userForm" id="userForm">
					...
					<td style="vertical-align:top;padding-left:2px;">
					   	<div class="nav-search">
									<span class="input-icon">
										<select class="chosen-select form-control"  id="nav-search-input" name="trainType" id="trainType" data-placeholder="学校类型查找" style="vertical-align:top;width: 120px;">
															<option value="">学校类型查找</option>														
															
														<option value="培训" <c:if test="${pd.trainType == '培训' }">selected</c:if>>培训</option>															
														<option value="民办" <c:if test="${pd.trainType == '民办' }">selected</c:if>>民办</option>
														<option value="公办" <c:if test="${pd.trainType == '公办' }">selected</c:if>>公办</option>
                        ...
                        <td>
									<div class="nav-search">
									<span class="input-icon">
										<select class="chosen-select form-control"  id="nav-search-input" name="w_id" id="w_id" data-placeholder="岗前培训学校" style="vertical-align:top;width: 120px;">
															<option value="">岗前培训类别学校</option>
															<c:forEach items="${listPreJobs}" var="u">
																
															    <option value="${u.w_id}" <c:if test="${pd.w_id == u.w_id }">selected</c:if>>${u.w_name }</option>
															</c:forEach>
															<i class="ace-icon fa fa-search nav-search-icon"></i>
														  	</select>
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
							<thead>
								...
								<!-- 
									...
									<th class="center">包分配</th>
									<th class="center">平均周期</th>
									<th class="center">平均就业起薪</th>									
									<th class="center">近三年平均就业率</th>
							...
							<!-- 开始循环 -->	
							<c:choose>
								<c:when test="${not empty listSchoolTrain}">
									<c:forEach items="${listSchoolTrain}" var="school" varStatus="vs">
										<tr>
										<!-- 
										<td class='center' style="width: 30px;">
											...
											<td class="center" style="cursor:pointer;color:blue;" onClick="toSchoolTrainCurriculumDetail('${school.school_id}')">培训课程</td>
										   ...
										   <c:if test="${school.is_distribution_work==0 }">
										   <td class="center">否</td>
										   </c:if>
										    <c:if test="${school.is_distribution_work==1 }">
										   <td class="center">是</td>
										   </c:if>
										   <td class="center">${school.average_training_cycle }</td>
										   <td class="center">${school.employment_salary }</td>
										   <td class="center">${school.employment_rate }</td>
...
function toSchoolTrainCurriculumDetail(school_id){
	   top.jzts();
		 var diag = new top.Dialog();
		 diag.Drag=true;
		 diag.Title ="地址详情";
		 diag.URL = '<%=basePath%>train/showlTrainCurriculumDetailByS_id?school_id='+school_id;
		 diag.Width = 800;
		 diag.Height=350;
