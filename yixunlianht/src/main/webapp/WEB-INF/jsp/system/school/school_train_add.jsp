									...
									<div class="widget-body">
										<div class="widget-main">
											<div class="step-content row-fluid position-relative">
												<div id="zhongxin">
												<form  action="<%=basePath%>train/saveTrainSchoolInfo.do" name="menuForm" id="menuForm" method="post" class="form-horizontal">
													<textarea name="CONTENT" id="CONTENT" style="display: none"></textarea>
													<input type="hidden" value="028-82687920" name="phone">
													<input type="hidden" value="培训" name="type">
													<input type="hidden" value="是" name="is_distribution_work">
													<table style="width: 100%;" id="xtable">
						
														<tr id="users">
															<td style="padding-top: 10px;">培训机构名称:<input
																type="text" name="name" id="name" value=""
																maxlength="50" placeholder="这里输入培训机构名称" title="培训机构名称"
														...
														<tr >
															<td style="padding-top: 10px;">培训机构电话:<input
																type="text" name="school_phone" id="school_phone" value=""
																maxlength="50" placeholder="这里输入培训机构电话" title="培训机构电话"
																style="width: 240px; margin-left: 20px; margin-right: 10px;" />
																校长:<input type="text" placeholder="这里输入培训机构校长"  id="principal"  name="principal"  /> 
															</td>
														</tr>
														
														<tr>
														 <td style="padding-top: 10px;">培训就业类型:
														 <select  name="w_id" id="w_id" data-placeholder="选择就业类型" style="vertical-align:top;width: 120px;">
															<option value="">选择就业类型</option>
															<c:forEach items="${listPreJobType}" var="perJob">
																<option value="${perJob.w_id}">${perJob.w_name }</option>
															</c:forEach>
														  	</select>&nbsp;&nbsp;&nbsp;														 
														 机构所属类型
														   <input
																type="text" name="stype" id="stype" value=""
																maxlength="50" placeholder="例如：计算机类，财经类，综合类等" title="培训类型"
																style="width: 240px; margin-left: 20px; margin-right: 10px;" />
														...
														<td style="padding-top: 10px;">是否包分配工作:
													          ...
													          &nbsp;&nbsp;&nbsp; 近三年平均就业率:			
																...
																<input
																type="text" name="employment_rate" id="employment_rate" 
														   ...
														   <tr>
																 <td style="padding-top: 10px;">平均就业起薪:<input placeholder="例如：3000(单位：元)" 
																type="number" name="employment_salary" id="employment_salary" value=""
																 title="专业数量" style="width: 240px; margin-left: 20px; margin-right: 10px;" />
																平均周期:<input type="number" placeholder="30天或60天"  id="average_training_cycle"  name="average_training_cycle"  />天数 
															</td>
														</tr>
															<tr>
																 <td style="padding-top: 10px;">专业数量:<input
																type="number" name="major_num" id="major_num" value="" 
																maxlength="50" title="专业数量" style="width: 240px; margin-left: 20px; margin-right: 10px;" />
																培训机构等级:<input type="text" placeholder="例如:本科，专科"  id="grade"  name="grade"  /> 
															</td>
														</tr>
														<tr>
   	...
   	//保存
		function save() {
			...
			if($("#school_phone").val()==""){
				...
				$("#school_phone").focus();
				return false;
			}else if($("#school_phone").val().length != 11 ){
				$("#school_phone").tips({
					side:3,
		            msg:'手机号格式不正确',
		            bg:'#AE81FF',
			...
			if($("#employment_rate").val()==""){
				$("#employment_rate").tips({
					side:3,
		            msg:'这里输入近三年平均就业率',
		            bg:'#AE81FF',
		            time:3
		        });
				$("#employment_rate").focus();
				return false;
			}
			if($("#employment_salary").val()==""){
				$("#employment_salary").tips({
					side:3,
		            msg:'这里输入平均就业起薪',
		            bg:'#AE81FF',
		            time:3
		        });
				$("#employment_salary").focus();
				return false;
			}
			
			if($("#average_training_cycle").val()==""){
				$("#average_training_cycle").tips({
					side:3,
		            msg:'这里输入平均周期',
		            bg:'#AE81FF',
		            time:3
		        });
				$("#average_training_cycle").focus();
				return false;
			}
