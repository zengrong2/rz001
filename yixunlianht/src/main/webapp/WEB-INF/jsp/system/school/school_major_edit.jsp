		...
		<div class="main-content">
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
								<form action="school/major/editSm.do" name="menuForm" id="menuForm" method="post">
									...
									<table id="table_report" class="table table-striped table-bordered table-hover" >
										<tr>
											...
											<c:forEach items="${mtList}" var="mt">
												<option value="${mt.type_id }" <c:if test="${pd.subject_type==mt.type_name }">selected</c:if>>${mt.type_name }</option>
											</c:forEach>
											</select>
											<input type="hidden" id="subject_type" name="subject_type" value="${pd.subject_type }" />
                                        ...
                                        <tr>
							              ...
							              <input type="hidden" id="major_name" name="major_name" value="${pd.major_name }" />
							              <input type="hidden" id="type_id" name="type_id"  value="${pd.type_id }"/>
											</td>
                                         </tr>
                                         
                                         <tr>
											<td style="width:79px;text-align: right;padding-top: 13px;">招生对象:</td>
											<td><input type="text" name="zsdx" id="zsdx"  value="${pd.zsdx }"  maxlength="32" placeholder="这里输入招生对象" title="招生对象" style="width:98%;"/></td>
										</tr>
										<tr>
											<td style="width:79px;text-align: right;padding-top: 13px;">就读年限:</td>
											<td><input type="text" name="year" id="year"  value="${pd.year }"  maxlength="32" placeholder="这里输入年限" title="年限" style="width:98%;"/></td>
										</tr>
										
										<tr>
											<td style="width:79px;text-align: right;padding-top: 13px;">就读费用:</td>
											<td><input type="number" name="money" id="money"  value="${pd.money/100 }" maxlength="32" placeholder="这里输入就读费用" title="就读费用"  style="width:98%;"/></td>
										</tr>
										
										<tr>
											<td style="width:79px;text-align: right;padding-top: 13px;">备注:</td>
											<td><input type="text" name="remark" id="remark"  value="${pd.remark }" maxlength="32" placeholder="备注" title="备注"  style="width:98%;"/></td>
										</tr>
										<tr>
...
<script type="text/javascript">
	$(top.hangge());
	var two_type='${pd.two_type}';
	var major_name='${pd.major_name}';
	var mid='${pd.type_id}';
	var ddd='${pd}';
	
	var school_name='${pd.school_name}';
				...
				$.ajax({
					...
					success:function(ret){
						...
						for (var i = 0; i < ret.length; i++) {
							...
							if(two_type==ret[i].type_name){
								_this.typeId = ret[i].type_id;
				...
				var _this=this;
				 var url2="<%=basePath%>school/major/getMajorTypeByPid?pid="+typeId;
					_this.$http.get(url2).then(function(data) {
						_this.ma = data.body;
						console.log(_this.ma)
						for (var i = 0; i < _this.ma.length; i++) {
							if(major_name==_this.ma[i].type_name){
								
								_this.pid=_this.ma[i].type_id;
							}
						}
