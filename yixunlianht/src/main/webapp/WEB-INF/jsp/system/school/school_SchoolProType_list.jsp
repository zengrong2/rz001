							...
							<tr>
								<td>
									...
									<span class="input-icon">
										<input class="nav-search-input" autocomplete="off" id="nav-search-input" type="text" name="keywords" value="${pd.keywords }" placeholder="这里输入升学类别名称" />
									...
									<c:forEach items="${schoolProTypeList}" var="ts" varStatus="vs">
										<tr>
											...
											<td class='center' style="width: 30px;">
												<label><input type='checkbox' name='ids' value="${ts.c_id }"   class="ace"/><span class="lbl"></span></label>
											</td>
...
function add(school_id){
	 ...
	 diag.Drag=true;
	 diag.Title ="新增";
	 diag.URL = '<%=basePath%>schoolProType/goAddSchoolPreType.do?school_id='+school_id;
	 diag.Width = 500;
	 diag.Height = 300;
	 diag.CancelEvent = function(){ //关闭事件
		 window.location.reload();
