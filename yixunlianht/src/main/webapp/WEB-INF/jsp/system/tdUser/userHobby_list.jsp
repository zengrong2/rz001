			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="hobby/listHobbyType.do" method="post" name="userForm" id="userForm">
							...
							<tr>
								<td>
									...
									<span class="input-icon">
										<input class="nav-search-input" autocomplete="off" id="nav-search-input" type="text" name="keywords" value="${pd.keywords }" placeholder="这里输入兴趣爱好名称" />
					   ...
					   --%>
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
							...
							<!-- 开始循环 -->	
							<c:choose>
								<c:when test="${not empty listHobby}">
									<c:forEach items="${listHobby}" var="ts" varStatus="vs">
										<tr>
										
											<td class='center' style="width: 30px;">
												<label><input type='checkbox' name='ids' value="${ts.hobby_id }"   class="ace"/><span class="lbl"></span></label>
											</td>
											<td class="center">${vs.index+1}</td>
											<td class='center'>${ts.hobby_name}</td>
										...
										<td class='center'>${ts.created}</td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-xs btn-success" title="编辑" onclick="editT('${ts.hobby_id}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													<a class="btn btn-xs btn-danger" onclick="delHobby('${ts.hobby_id}');">
...
//删除
function delHobby(hobby_id){
	bootbox.confirm("确定要删除吗?", function(result) {
		if(result) {
			top.jzts();
			var url = "<%=basePath%>hobby/delHobbyById.do?hobby_id="+hobby_id;
			$.get(url,function(data){
				window.location.reload();
			});
		};
...
function add(){
	 ...
	 diag.Drag=true;
	 diag.Title ="新增";
	 diag.URL = '<%=basePath%>hobby/goAddHobby.do';
	 diag.Width = 600;
	 diag.Height = 300;
	 diag.CancelEvent = function(){ //关闭事件
		 window.location.reload();
...
function editT(hobby_id){
	 top.jzts();
	 var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="修改";
	 diag.URL = '<%=basePath%>hobby/goEditHobby.do?hobby_id='+hobby_id;
	 diag.Width = 500;
	 diag.Height = 300;
	 diag.CancelEvent = function(){ //关闭事件
		 window.location.reload();
