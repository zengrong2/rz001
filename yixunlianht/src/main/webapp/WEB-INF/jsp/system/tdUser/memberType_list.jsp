					...
					<div class="row">
						...
						<!-- 	<tr> -->
								<%-- <td>
									...
									<span class="input-icon">
										<input class="nav-search-input" autocomplete="off" id="nav-search-input" type="text" name="keywords" value="${pd.keywords }" placeholder="这里输入会员等级名称" />
					   ...
					   --%>
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center" style="width:60px;">编号</th>
									    <th class='center'>会员等级名称</th>
									    <th class='center'>会员使用天数</th>
									    <th class='center'>会员所需金额</th>
							...
							<!-- 开始循环 -->	
							<c:choose>
								<c:when test="${not empty listMemberType}">
									<c:forEach items="${listMemberType}" var="ts" varStatus="vs">
										<tr>
										
											<td class='center' style="width: 30px;">
												<label><input type='checkbox' name='ids' value="${ts.utype_id }"   class="ace"/><span class="lbl"></span></label>
											</td>
										<td class="center">${vs.index+1}</td>
										<td class='center'>${ts.user_type}</td>
										<td class='center'>${ts.user_day}</td>
										<td class='center'>${ts.utype_charge}</td>
										...
										${ts.countTdUser }
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-xs btn-success" title="编辑" onclick="editT('${ts.utype_id}');">
...
function toUserDetail(user_id){
   top.jzts();
	 ...
	 diag.Drag=true;
	 diag.Title ="用户详情";
	 diag.URL = '<%=basePath%>clientUser/showUserDetailById?user_id='+user_id;
	 diag.Width =500;
	 diag.Height=283;
...
function delUser(user_id){
	bootbox.confirm("确定要删除吗?", function(result) {
		if(result) {
			top.jzts();
			var url = "<%=basePath%>mUser/delUserByUid.do?user_id="+user_id;
			$.get(url,function(data){
				window.location.reload();
			});
		};
...
function add(){
	 ...
	 diag.Drag=true;
	 diag.Title ="新增";
	 diag.URL = '<%=basePath%>clientUser/goAddUser.do';
	 diag.Width = 600;
	 diag.Height = 300;
	 diag.CancelEvent = function(){ //关闭事件
		 window.location.reload();
...
function editT(utype_id){
	 top.jzts();
	 var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="修改";
	 diag.URL = '<%=basePath%>memberType/goEditMemberType.do?utype_id='+utype_id;
	 diag.Width = 500;
	 diag.Height = 510;
	 diag.CancelEvent = function(){ //关闭事件
		 window.location.reload();
