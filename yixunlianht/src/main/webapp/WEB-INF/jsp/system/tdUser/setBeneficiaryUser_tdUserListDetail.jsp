				...
				<div class="page-content">
					<div class="row">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
							...
							<!-- 开始循环 -->	
							<c:choose>
								<c:when test="${not empty tdUserList}">
									<c:forEach items="${tdUserList}" var="ts" varStatus="vs">
										<tr>
										
											<%-- <td class='center' style="width: 30px;">
												<label><input type='checkbox' name='ids' value="${ts.userId }"   class="ace"/><span class="lbl"></span></label>
											</td> --%>
											<td class="center">${vs.index+1}</td>
											<td class='center'>${ts.uNickname}</td>
										<td class='center'>${ts.memberid }</td>
										<td class='center'>${ts.uPhone }</td>
										<c:if test="${ts.utypeId=='1'||ts.utypeId==''||ts.utypeId==null}">
										<td class='center'>普通用户</td>
										</c:if>
										<c:if test="${ts.utypeId=='2'}">
										<td class='center'>普通客户经理</td>
										</c:if>
										<c:if test="${ts.utypeId=='3'}">
										<td class='center'>VIP客户经理</td>
										</c:if>
										<c:if test="${ts.utypeId=='4'}">
										<td class='center'>VIP体验客户经理</td>
										</c:if>
										<c:if test="${ts.utypeId=='5'}">
										<td class='center'>VIP试用客户经理</td>
										</c:if>
										<c:if test="${ts.utypeId=='6'}">
										<td class='center'>钻石客户经理</td>
										</c:if>
										<%-- <td class='center'>${ts.userRefereeName}</td>
										<td class='center'>${ts.userManagerName}</td> --%>
										<%-- <td class="center" >
										<div style="cursor:pointer;color:blue;margin-left:10px;" onClick="toUserOverJoinDetail('${ts.userId}')">查阅</div>										
