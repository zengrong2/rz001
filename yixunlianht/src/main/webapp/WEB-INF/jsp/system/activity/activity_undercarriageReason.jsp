...
<body>
							...
							<div class="span12">
								<div class="widget-box">
					         	...
					         	<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
						            	<thead>
							     	<tr>
									    <th class='center'>被下架活动</th>
									    <th class='center'>下架人</th>
									   <!--  <th class='center'>下架人编号</th>			 -->	
										<th class='center'>下架原因</th>
							...
							<c:choose>
								<c:when test="${not empty listTdActivity}">
									<c:forEach items="${listTdActivity}" var="ts" varStatus="vs">
							            ...
							            <c:if test="${ts.onlineStatus ==1}">
										<td class='center'>主办方</td>
										</c:if>
										<c:if test="${ts.onlineStatus ==3}">
										<td class='center'>管理员</td>
										</c:if>
										<c:if test="${ts.onlineStatus ==''||ts.onlineStatus==null}">
										<td class='center'></td>
										</c:if>
										<%-- <td class='center'>${ts.reportUserPhone}</td> --%>
									    <td class='center'>${ts.undercarriageReason}</td>
										<td class='center'>${ts.created}</td>
										<td class="center">
										<c:if test="${ts.onlineStatus==1||ts.onlineStatus==3 }">				
									    <a class="label label-info btn-danger" onclick="isUpperShelf('${ts.onlineStatus}','${ts.activityId }');">上架</a>
									   
										</c:if>
										<c:if test="${ts.onlineStatus==0 }">
										<span style="cursor:pointer;color:blue;margin-left:10px;" >已上架</span>
										</c:if>
	...
	<script type="text/javascript">	   
		...
		function isUpperShelf(onlineStatus,activityId){	
			...
			bootbox.confirm("确定要进行此操作吗?", function(result) {				
				    if(result){
			          ...
			          $.ajax({
		                  type:'POST',
		                  url:'<%=basePath%>activity/updateActivityStateUnder.do',
		                  data:{
