...
<body>
							...
							<div class="span12">
								<div class="widget-box">
					         	...
					         	<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
						            	<thead>
							     	<tr>
									    <th class='center'>被举报活动</th>
									    <th class='center'>举报人/会员号</th>
									    <th class='center'>举报人电话</th>				
										<th class='center'>举报原因</th>
							...
							<c:choose>
								<c:when test="${not empty listTdReport}">
									<c:forEach items="${listTdReport}" var="ts" varStatus="vs">
										...
										<td class='center'>${ts.reportUserName}
										<c:if test="${ts.reportUserMemberId !=null}">
										/${ts.reportUserMemberId }
										</c:if>
										</td>
										<td class='center'>${ts.reportUserPhone}</td>
									    <td class='center'>${ts.reportContext}</td>
										<td class='center'>${ts.created}</td>
										<td class="center">
										<c:if test="${ts.isExamine==0||ts.isExamine==''||ts.isExamine==null }">				
									    <a class="label label-info btn-danger" onclick="isExamineReport('${ts.reportId}','${ts.isExamine}','${ts.activityId }');">未审核</a>
	...
	<script type="text/javascript">	   
		...
		function isExamineReport(reportId,isExamine,activityId){	
			...
			bootbox.confirm("确定要进行此操作吗?", function(result) {				
				    if(result){
			          ...
			          $.ajax({
		                  type:'POST',
		                  url:'<%=basePath%>report/updateReportState.do',
		                  data:{
		                	  reportId:reportId,
		                	  isExamine:isExamine,
