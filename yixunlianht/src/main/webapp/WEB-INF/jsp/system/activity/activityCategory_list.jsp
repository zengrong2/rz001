			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="activityType/goListactivityType.do" method="post" name="userForm" id="userForm">
							...
							<tr>
								<td>
									...
									<span class="input-icon">
										<input class="nav-search-input" autocomplete="off" id="nav-search-input" type="text" name="keywords" value="${pd.keywords }" placeholder="这里输入活动类型名称" />
					   ...
					   --%>
						...
						<div style="margin-top:0px;margin-left:300px;margin-top:-30px;">
							<span style="width:150px;font-weight: bold;">
							<button style="border-radius: 20px;" type="button" class="btn btn-primary btn-sm btn-info" onClick="isSpecialACUser()">
							授权人数：${countSpecialACUser }</button></span>
						</div>
						<!-- 检索  -->
						<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center" style="width:60px;">编号</th>
									    <th class='center'>活动类型名称</th>
									    <th class='center'>在线情况</th>
									    <th class='center'>特殊类别</th>
							...
							<!-- 开始循环 -->	
							<c:choose>
								<c:when test="${not empty listActivitycategory}">
									<c:forEach items="${listActivitycategory}" var="ts" varStatus="vs">
										<tr>
										
											<%-- <td class='center' style="width: 30px;">
												<label><input type='checkbox' name='ids' value="${ts.activitycategory_id }"   class="ace"/><span class="lbl"></span></label>
											</td> --%>
											<td class="center">${vs.index+1}</td>
											<td class='center'>${ts.activitycategory_name}</td>
											<c:if test="${ts.is_online ==0 || ts.is_online=='' ||ts.is_online==null}">
											<td class='center'>下线</td>
											</c:if>
											<c:if test="${ts.is_online ==1}">
											<td class='center'>上架</td>
											</c:if>
										<c:if test="${ts.is_special ==0}">
											<td class='center'></td>
										</c:if>
										<c:if test="${ts.is_special ==1}">
										...
										<td class='center'>${ts.created}</td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.del == 1 }">
											  <c:if test="${ts.is_online==0 ||ts.is_online==null||ts.is_online==''}">&nbsp;&nbsp;
										      <a class="label label-error arrowed" onclick="changeState('${ts.activitycategory_id}','${ts.is_online}');">上架</a>
										      </c:if>
										      <c:if test="${ts.is_online==1}">
										      &nbsp;&nbsp;<a class="label label-danger arrowed" onclick="changeState('${ts.activitycategory_id}','${ts.is_online}');">下架</a>
											  </c:if>
													<a class="btn btn-xs btn-success" title="编辑" onclick="editT('${ts.activitycategory_id}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													<%-- <a class="btn btn-xs btn-danger" onclick="delActivityType('${ts.activitycategory_id}');">
...
function isSpecialACUser(user_id){
	
	   top.jzts();
		 var diag = new top.Dialog();
		 diag.Drag=true;
		 diag.Title ="特殊活动类别授权用户详情";
		 diag.URL = '<%=basePath%>activityType/showIsSpecialACUser';
		 diag.Width = 800;
		 diag.Height =450;
...
function changeState(activitycategory_id,is_online){	
	bootbox.confirm("确定要进行此操作吗?", function(result) {
		    if(result){
	          ...
	          $.ajax({
                  type:'POST',
                  url:'<%=basePath%>activityType/updateActivityCategoryState.do',
                  data:{
                	  activitycategory_id:activitycategory_id,
                	  is_online:is_online
