							...
							<c:choose>
								<c:when test="${not empty tdUenrollandactivityList}">
									<c:forEach items="${tdUenrollandactivityList}" var="ts" varStatus="vs">
										...
										<td class='center'>${ts.mustWriteFillInvitingUser }/${ts.eventSharerMemberid }</td>
										<td class='center'>
										...
										<td class='center'>
						                <a class="label label-success arrowed" >未成交/${ts.transactionNum }</a>						
										</td>
										</c:if>
										<c:if test="${ts.dealStatus==1 }">
										<td class='center'>
						                <a class="label label-success arrowed" >已成交/${ts.transactionNum }</a>						
										</td>
										</c:if>
										<c:if test="${ts.paymentStatus==''||ts.paymentStatus==null||ts.paymentStatus==0 }">
										<td class='center'>${ts.paymentNum }/未发放</td>
										</c:if>
										<c:if test="${ts.paymentStatus==1 }">
										<td class='center'>${ts.paymentNum }/已发放</td>
										</c:if>
