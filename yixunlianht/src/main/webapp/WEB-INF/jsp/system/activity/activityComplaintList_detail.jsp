...
<body>
							...
							<div class="span12">
								<div class="widget-box">
					         	...
					         	<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
						            	<thead>
							     	<tr>
									    <th class='center'>被投诉活动</th>
									    <th class='center'>投诉人/会员号</th>
									    <th class='center'>投诉人电话</th>				
										<th class='center'>投诉原因</th>
							...
							<c:choose>
								<c:when test="${not empty listTdComplaint}">
									<c:forEach items="${listTdComplaint}" var="ts" varStatus="vs">
							            ...
							            <td class='center'>${tdActivity.activityName}
							            </td>
										<td class='center'>${ts.complaintUserName}
										<c:if test="${ts.complaintUserMemberId !=null}">
										/${ts.complaintUserMemberId }
										</c:if>
										</td>
										<td class='center'>${ts.complaintUserPhone}</td>
									    <td class='center'>${ts.complaintContext}</td>
										<td class='center'>${ts.created}</td>
										<td class='center'>
										<c:if test="${ts.isExamine==0||ts.isExamine==''||ts.isExamine==null }">				
									    <a class="label label-info btn-danger" onclick="isExamineReport('${ts.complaintId}','${ts.isExamine}','${ts.activityId }');">未审核</a>
									   
										</c:if>
										<c:if test="${ts.isExamine==1 }">
										<span style="cursor:pointer;color:blue;margin-left:10px;" >已审核</span>
										</c:if>
	...
	function isExamineReport(complaintId,isExamine,activityId){
		if(isExamine==""||isExamine==null){
			isExamine="0";
		}
		bootbox.confirm("确定要进行此操作吗?", function(result) {
			    ...
			    if(result){
		          ...
		          $.ajax({
	                  type:'POST',
	                  url:'<%=basePath%>complaint/updateComplaintState.do',
	                  data:{
