...
public class AdvertisementController extends BaseController{
	//这个注入的是通用mapper使用的service
		...
		public void delAdvertisementByAid(PrintWriter out) throws Exception {
			...
			FHLOG.save(Jurisdiction.getUsername(), Jurisdiction.getUsername()+"：删除首页广告->"+tdAdvertisementimage.getImgName());
...
*/				mv.addObject("pd", tdAdvertisementimage);				//放入视图容器
	...
	*/		ModelAndView mv = this.getModelAndView();
		...
		@ResponseBody
		public Object sendadvertisementAdd() throws Exception{
			PageData pd = new PageData();
			pd = this.getPageData();
	...
	//TYPE 为1为文本格式，为2为html格式
	    if("1".equals(TYPE)){
	    	...
	    	tdAdvertisementimage.setAdvertisementDetailsurl(pubActivityEditorImage(NewsController.editorFixImage(CONTENT)));
	    	tdAdvertisementimage.setImgUrl(img_url);
	    	
	    	int returnSaveResult =	service.save(tdAdvertisementimage);
	    	 if(returnSaveResult==1) {
	         	msg = "ok";
		FHLOG.save(Jurisdiction.getUsername(), Jurisdiction.getUsername()+"：添加首页广告->"+img_name);
		...
		public void updateAdvertisementState(PrintWriter out) throws Exception {
			...
			if(is_online_status==0) {
				...
				FHLOG.save(Jurisdiction.getUsername(), Jurisdiction.getUsername()+"上架首页广告"+tdAdvertisementimage.getImgName());
	...
	*/		ModelAndView mv = this.getModelAndView();
		...
		@ResponseBody
		public Object sendadvertisementEdit() throws Exception{
			PageData pd = new PageData();
			pd = this.getPageData();
			Map<String,Object> map = new HashMap<String,Object>();
			String msg = "";		//发送状态
	...
	//TYPE 为1为文本格式，为2为html格式
	    if("1".equals(TYPE)){
	    	...
	    	TdAdvertisementimage advertisementimage = service.queryById(id);
	    	int returnSaveResult =	service.updateSelective(tdAdvertisementimage);
	    	 if(returnSaveResult==1) {
	         	msg = "ok";
	    FHLOG.save(Jurisdiction.getUsername(), Jurisdiction.getUsername()+"：修改首页广告->"+advertisementimage.getImgName()+"改为：" +img_name);
