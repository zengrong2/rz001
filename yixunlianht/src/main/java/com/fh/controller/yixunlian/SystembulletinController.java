...
*/		ModelAndView mv = this.getModelAndView();
	...
	public Object sendSystembulletin() throws MessagingException{
		...
		String shareTitle = pd.getString("shareTitle");	
...
//TYPE 为1为文本格式，为2为html格式
    if("1".equals(TYPE)){
    	...
    	tdSystembulletin.setShareTitle(shareTitle);
    	tdSystembulletin.setShareSummary(shareSummary);
    	tdSystembulletin.setSystembulletinId(systembulletinId);
    	tdSystembulletin.setSystembulletinUrl(pubActivityEditorImage(NewsController.editorFixImage(CONTENT)));
