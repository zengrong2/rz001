...
//活动全选删除
	...
	public void delAllActivity(String position_IDS) {
		...
		for(int i=0;i<ArrayPosition_IDS.length;i++) {
			Integer a = Integer.parseInt(ArrayPosition_IDS[i]);
			list.add(a);
		}
		TdActivity tdActivity = new TdActivity(); 
    ...
    //主办方活动发布量
	public int getOrganizerPubCountActivity(String userId) {
		// TODO Auto-generated method stub
		int count = 0;
		TdActivity tdActivity = new TdActivity();
		tdActivity.setUserId(userId);
		count = super.queryTotalCount(tdActivity);
		
		/*Example example = new Example(tdActivity.getClass());
		example.createCriteria().andEqualTo("userId", userId).andEqualTo("onlineStatus", value);
		*/
		return count;
	}
}
